...
export default class StormCollection {
  ...
  update (query, callback) {
    AsyncStorage.getItem(this.collection, (err, res) => {
      res = !!res ? this.string(res) : []
      let paramsLength = Object.keys(query.where).length

      res.forEach(elemt => {
        let counter = 0

        for (let key in elemt) if (elemt[key] === query.where[key]) counter++
        if (!(counter === paramsLength)) for (let key in query.set) elemt[key] = query.set[key]
